...
main ( int argc, char *argv[] )
{
    ...
    ret = ftruncate(sfd,sizeof(int)*3);
    if(!ret){
    ...
    shared_memory_ptr = mmap(NULL,sizeof(int)*3,PROT_WRITE|PROT_READ,MAP_SHARED,sfd,0);
    ...
    if(!my_pid){
// child process
        if(execl("./sm-proxy.py","./sm-proxy.py","-k",my_string,"-t","0",NULL)<0){
            printf("error\n");
            printf("error is %d\n",errno);
        }
    }else{
